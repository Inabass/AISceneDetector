...
```powershell
curl.exe "http://127.0.0.1:8000/api/v1/settings"
```
...
Invoke-RestMethod -Method Put `
  ...
  -ContentType "application/json" `
  -Body '{"values":{"default_padding_sec":1.0,"default_merge_gap_sec":1.5}}'
```

Web UIでは、`設定` セクションから以下を変更できます。

- frame interval既定値
- training / detection batch size
- smoothing window
- merge gap
- padding
...
設定はDBに保存され、新しい特徴量抽出、検出、シーン区間生成、feedback再学習に反映されます。
...
- thresholdのモデル別チューニングUIはまだ最小限です。
