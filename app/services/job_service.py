...
class JobService:
    ...
    def retry(self, job_id: int) -> Job:
        ...
        if source.status not in {"failed", "cancelled"}:
            raise ValidationAppError(
                ...
                detail={"job_id": job_id, "status": source.status},
            )
    ...
    def recover_interrupted_jobs(self) -> int:
        jobs = self.repository.list_by_status({"running", "cancel_requested"})
        recovered = 0
        with UnitOfWork(self.db):
            for job in jobs:
                if job.status == "cancel_requested":
                    job.status = "cancelled"
                    job.current_step = "cancelled_after_restart"
                    job.finished_at = datetime.now(timezone.utc)
                else:
                    job.status = "queued"
                    job.current_step = "requeued_after_restart"
                recovered += 1
        return recovered
    ...
    def list_logs(self, job_id: int, limit: int = 200) -> list[JobLog]:
        ...
        return self.repository.list_logs(job_id, limit)
